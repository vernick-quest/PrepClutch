...
import { BADGES, evaluateBadges, type Badge, type BadgeStats } from '@/lib/hspt-badges-config'
...
function BadgeCard({ badge, earned }: { badge: Badge; earned: boolean }) {
  ...
  return (
    <div onClick={() => earned && setOpen(o => !o)}
      style={{
...
export default function BestiaryClient({ displayName, avatarColor }: Props) {
  ...
  const earnedIds = evaluateBadges({ ...stats, earnedBadgeIds: [] }).map(b => b.id)
  const allBadges = Object.values(BADGES)
  const filtered  = catFilter === "All" ? allBadges : allBadges.filter(b => b.category === catFilter)
  const pct       = Math.round((earnedIds.length / allBadges.length) * 100)
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{display:"flex",justifyContent:"center",gap:32,marginBottom:22}}>
          {([[earnedIds.length,"#c084fc","Earned"],[allBadges.length,"#374151","Total"],[`${pct}%`,"#fbbf24","Complete"]] as [string|number,string,string][]).map(([v,c,l]) => (
      ...
      {earnedIds.length === 0 && (
        <div style={{textAlign:"center",padding:"48px 24px",color:"#374151",fontSize:14}}>
          Complete a challenge in Challenge Mode to start earning badges.
        </div>
      )}
      ...
      <div style={{maxWidth:920,margin:"28px auto 0",padding:"0 24px"}}>
        {CATEGORY_ORDER.filter(cat => catFilter==="All" || catFilter===cat).map(cat => {
          ...
          const catEarned = catBadges.filter(b => earnedIds.includes(b.id)).length
          return (
